            ...
            for symbol in trading_symbols:
                ...
                for trading_type in trading_types:
                    ...
                    if any_signal:
                        ...
                        # Проверяем баланс только для ограничения новых позиций
                        if user_balance < min_trade_usdt:
                            print(f"[INFO] Пользователь {uid}: баланс {user_balance:.2f} USDT < {min_trade_usdt} USDT. Новые позиции не открываются.")
                            # Пропускаем создание новых ордеров, но позволяем закрытие существующих
                            can_create_new_orders = False
                        else:
                            can_create_new_orders = True
                        
                        # Создаем новые ордера только если баланс достаточный
                        if can_create_new_orders:
